...
class _MarketPricesPageState extends State<MarketPricesPage> {
  ...
  Future<void> _fetchMarketData(String state) async {
    setState(() {
      _isLoading = true;
      _errorMessage = null;
      _marketData = [];
    ...
    try {
      final url =
          'https://api.data.gov.in/resource/9ef84268-d588-465a-a308-a864a43d0070?api-key=$_apiKey&format=xml&filters[state.keyword]=$state';
      ...
      body: Container(
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [const Color.fromARGB(255, 62, 148, 36), Colors.teal.shade300],
          ),
        ),
        child: SafeArea(
          child: Column(
  ...
  Widget _buildContent() {
    if (_errorMessage != null) {
      ...
      return const Center(
        child: Text(
          'No market data available for this state.',
          style: TextStyle(fontSize: 16, color: Colors.grey),
          textAlign: TextAlign.center,
        ),
      );
